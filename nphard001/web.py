...
def MakoContextFilter(template_var_in: Optional[Dict]=None):
    ...
    return template_var
def MakoRender(template_string: str, template_var_in: dict, lookup_dir: list=['mako']):
...
def MakoRenderFile(template_path: str, template_var_in: dict, lookup_dir: list=['mako']):
    lookup = TemplateLookup(directories=lookup_dir, **MakoExtraOption())
    template_var = MakoContextFilter(template_var_in)
    return Template(
...
class MakoManager:
    r'''A configurable mako API wrapper'''
    def __init__(self, lookup_main_dir_list):
        if type(lookup_main_dir_list)==type('mako'):
